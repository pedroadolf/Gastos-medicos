  ...
  return (
    <div className={`relative flex items-center justify-center gap-12 mb-24 ${isRight ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
      ...
      >
        <div className="gmm-pill-card group">
          <div className="flex justify-between items-start mb-6">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-gmm-text/40 mb-1">{title}</p>
          ...
          {/* SVG Area Chart (Clinical curve style) */}
            ...
            <svg viewBox="0 0 200 60" className="w-full h-full drop-shadow-sm">
                <defs>
                   <linearGradient id={`grad-${title}`} x1="0%" y1="0%" x2="0%" y2="100%">
                      <stop offset="0%" style={{ stopColor: '#FFAA00', stopOpacity: 0.2 }} />
                ...
                <motion.path 
                  ...
                  strokeLinecap="round"
                />
                <path d="M0 50 Q 25 45, 50 20 T 100 35 T 150 10 T 200 30 V 60 H 0 Z" fill={`url(#grad-${title})`} />
            </svg>
            <div className="absolute inset-x-0 bottom-0 flex justify-between px-2 text-[9px] font-bold text-gmm-text/20 uppercase tracking-widest">
              <span>Ago</span><span>Set</span><span>Oct</span><span>Nov</span><span>Dic</span>
            </div>
          </div>

          <p className="mt-6 text-[11px] font-medium text-gmm-text/50 uppercase tracking-widest leading-relaxed">
            {subtext}
          </p>
        </div>
      </motion.div>
      ...
      <div className={`hidden md:block w-[48%] ${isRight ? 'text-left' : 'text-right'}`}>
         ...
         <p className="text-[11px] font-bold text-gmm-text/30 uppercase tracking-[0.2em] max-w-xs ml-auto">
    ...
    <div className="max-w-6xl mx-auto py-10 px-6">
      <div className="mb-20">
        ...
        <p className="text-[11px] font-bold text-gmm-text/40 uppercase tracking-widest mt-3">
          Monitoreo proactivo de siniestralidad y eficiencia del gasto médico.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24">
         {[
           ...
           <div key={i} className="bg-white/60 backdrop-blur-sm rounded-[40px] p-8 border border-white shadow-xl shadow-black/5 text-center">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gmm-text/40 mb-3">{stat.label}</p>
              <h4 className={`text-4xl font-black tracking-tighter ${stat.color || 'text-gmm-text'}`}>{stat.val}</h4>
              <p className="text-[10px] font-black text-gmm-accent uppercase tracking-widest mt-2">{stat.sub}</p>
           </div>
         ))}
      </div>
      ...
      <div className="gmm-pill-card mt-20 text-center">
         <h4 className="text-sm font-black text-gmm-text uppercase tracking-[0.4em] mb-6">Estado del Motor de Gobernanza</h4>
         <div className="flex justify-center gap-2 mb-8">
            {[1,2,3,4,5,6,7,8].map(i => (
              <div key={i} className={`w-3 h-10 rounded-full ${i < 7 ? 'bg-gmm-accent' : 'bg-gmm-text/10'}`} />
            ))}
         </div>
         <p className="text-[10px] font-black text-gmm-text-muted uppercase tracking-widest flex items-center justify-center gap-2">
           <div className="w-2 h-2 rounded-full bg-gmm-success" /> Todos los sistemas de auditoría en línea
         </p>
      </div>
    </div>
  );
